        ...
        <div className="container mx-auto max-w-7xl px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button 
              ...
              className="rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold h-11 px-6 shadow-lg shadow-indigo-100 transition-all active:scale-95"
            >
          ...
          <aside className="lg:col-span-3 space-y-8">
            ...
            <motion.section variants={itemVariants} className="px-2">
               <div className="flex items-center justify-between mb-4">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Collective</h4>
                  <Badge variant="ghost" className="h-5 bg-muted text-muted-foreground font-black px-2">{project.members?.length || 0}</Badge>
               </div>
               <div className="flex -space-x-3">
                  {project.members?.slice(0, 5).map((m, i) => (
                    <div key={i} className="w-10 h-10 rounded-full border-2 border-background bg-primary/10 text-primary flex items-center justify-center text-[10px] font-black shadow-sm" title={m.name}>
                       {m.name?.slice(0, 1).toUpperCase()}
                    </div>
                  ))}
                  <button className="w-10 h-10 rounded-full border-2 border-background bg-muted text-muted-foreground flex items-center justify-center hover:bg-accent transition-colors">
                     <Plus size={14} />
                  </button>
               </div>
            </motion.section>
